...
// Common world events to simulate real data
...
// Helper function to find relevance between city and events
function findRelevantEvents(timezone: string, eventList: any[], locationKey: string = 'locations') {
  ...
  // Try to match city with locations in events
  return eventList.filter(event => {
    ...
    // Region match (try to match based on country/region)
...
// Helper to match based on regions/countries
function matchRegion(city: string, locations: string[]) {
  ...
  // Check if any location matches the city's regions
...
// Function to periodically send notifications for a given timezone
export function startContextualNotifications(timezone: string) {
  ...
  setTimeout(() => {
    ...
    // Use these to randomly send notifications
  ...
  const intervalId = setInterval(() => {
    // Similar logic but with lower probability to avoid notification spam
    const relevantWorldEvents = findRelevantEvents(timezone, majorWorldEvents);
    const relevantSportingEvents = findRelevantEvents(timezone, sportingEvents);
    const relevantWeatherEvents = findRelevantEvents(timezone, weatherEvents, 'regions');
