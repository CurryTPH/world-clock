...
// Scroll column to match the highlighted time
  const scrollColumnToTime = useCallback(
  ...
  React.useEffect(() => {
    if (highlightedTime) {
      selectedTimezones.forEach((_, index) => {
        scrollColumnToTime(index);
      });
    }
  }, [highlightedTime, scrollColumnToTime, selectedTimezones]);
...
// Check if this time slot represents the highlighted time
